...
export const CommonFields = {
  ...
  title: () =>
    z.string().min(1, 'Title is required').max(200, 'Title too long'),
  ...
  // Currency and money
  currency: () =>
    z.string().length(3, 'Currency must be 3 characters (e.g., USD)'),
  ...
  status: <T extends readonly [string, ...string[]]>(values: T) =>
    z.enum(values, {
      errorMap: () => ({
        message: `Status must be one of: ${values.join(', ')}`,
      }),
    }),
...
export function createCrudSchemas<T extends Record<string, z.ZodTypeAny>>(
  ...
  const createFields = Object.fromEntries(
    Object.entries(fields).filter(([key]) =>
      requiredFields.includes(key as keyof T),
    ),
  ) as { [K in keyof T]: T[K] }
...
export function validateSchema<T>(schema: z.ZodSchema<T>) {
  return {
    ...
    params: (data: unknown): T => {
      ...
      if (!result.success) {
        throw new Error(
          `Params validation failed: ${result.error.issues.map((i) => i.message).join(', ')}`,
        )
      }
      return result.data
    },
