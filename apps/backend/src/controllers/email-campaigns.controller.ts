...
export class EmailCampaignsController {
  ...
  async createCampaign(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const validated = createCampaignSchema.parse(req.body) as import('../services/email-campaigns.service').CreateCampaignData
  ...
  async sendCampaign(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!result.success) throw new ApiError(404, result.error || 'Send failed')
