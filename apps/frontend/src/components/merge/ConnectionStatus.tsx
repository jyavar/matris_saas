...
export function MergeConnectionStatus({ className = '', showDetails = false }: ConnectionStatusProps) {
  ...
  return (
    <div
      ...
      role="status"
      aria-live="polite"
      aria-label={`Estado de conexión de merge: ${config.label}`}
    >
      {/* Status Icon */}
      <span
        className={`
...
export function MergeConnectionStatusCompact({ className = '' }: { className?: string }) {
  const { state } = useMerge()
  ...
  return (
    <div
      ...
      `}
      role="status"
      aria-label={`Estado de merge: ${isHealthy ? 'Conectado' : 'Desconectado'}`}
      title={`Merge: ${isHealthy ? 'Conectado' : 'Desconectado'}`}
    >
...
export function MergeConnectionStatusBadge({ className = '' }: { className?: string }) {
  const { state } = useMerge()
  const { health, circuitBreakerOpen } = state
  ...
  return (
    ...
    >
      ...
      {isHealthy ? 'Merge OK' : 'Merge Error'}
    </span>
  )
} 