...
export function DocsConnectionStatus({ className = '', showDetails = false }: ConnectionStatusProps) {
  const { state } = useDocs()
  ...
  return (
    <div
      ...
      role="status"
      aria-live="polite"
      aria-label={`Estado de conexión de documentación: ${config.label}`}
    >
      {/* Status Icon */}
      <span
        className={`
...
export function DocsConnectionStatusCompact({ className = '' }: { className?: string }) {
  const { state } = useDocs()
  ...
  return (
    <div
      ...
      `}
      role="status"
      aria-label={`Estado de documentación: ${isHealthy ? 'Conectado' : 'Desconectado'}`}
      title={`Documentación: ${isHealthy ? 'Conectado' : 'Desconectado'}`}
    >
...
export function DocsConnectionStatusBadge({ className = '' }: { className?: string }) {
  const { state } = useDocs()
  const { health, circuitBreakerOpen } = state
