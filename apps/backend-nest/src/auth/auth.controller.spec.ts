...
interface MockAuthService {
  signUp: jest.MockedFunction<(credentials: SignUpDto) => Promise<AuthResponseDto>>
  signIn: jest.MockedFunction<(credentials: SignInDto) => Promise<SignInResponseDto>>
  refreshToken: jest.MockedFunction<(refreshData: RefreshTokenDto) => Promise<SignInResponseDto>>
...
describe('AuthController', () => {
  ...
  describe('refresh', () => {
    ...
    it('should handle refresh token service errors', async () => {
      ...
      await expect(controller.refresh(refreshData)).rejects.toThrow(
        errorMessage,
      )
      expect(authService.refreshToken).toHaveBeenCalledWith(refreshData)
    })
