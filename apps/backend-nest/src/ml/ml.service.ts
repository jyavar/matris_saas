...
export class MLService {
  ...
  async getGlobalMetrics(): Promise<Record<string, unknown>> {
    // Simulación de métricas globales
    return {
      total_models: 15,
      active_models: 12,
      total_datasets: 25,
  ...
  async getSystemStatus(): Promise<Record<string, unknown>> {
    ...
    return {
      ...
      last_maintenance: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), // 24h ago
      next_maintenance: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), // 7 days
    };
  }
