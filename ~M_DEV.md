<!-- STRATO MODULE HEADER
{
  "module": "DEV",
  "description": "Herramientas y utilidades de desarrollo para STRATO.",
  "paths": [
    "scripts/visibility-report.ts",
    "scripts/validate-traceability.ts",
    "scripts/validate-test-templates.ts",
    "scripts/validate-repo.ts",
    "scripts/validate-module.ts",
    "scripts/validate-env.ts",
    "scripts/taskmaster.prd.ts",
    "scripts/sync-modules-index.ts",
    "scripts/report-strato-defense.ts",
    "scripts/report-status.ts",
    "scripts/pre-commit-validation.ts",
    "scripts/night-watchman.ts",
    "scripts/init-project.ts",
    "scripts/generate-code.ts",
    "scripts/db-seed.ts",
    "scripts/create-module.ts",
    "scripts/check-react-imports.ts",
    "scripts/check-env.ts",
    "scripts/check-blindaje-real.ts",
    "scripts/audit-visibility.ts",
    "scripts/audit-module.ts",
    "scripts/audit-checklist.ts",
    "scripts/angel-vigilante.ts",
    "scripts/audit/audit-system.ts",
    "scripts/agents/strato.logic.ts",
    "scripts/agents/context-watchdog.watch.ts",
    "scripts/agents/context-watchdog.ts",
    "scripts/agents/security/audit.ts",
    "scripts/agents/refactor/autofix.ts"
  ],
  "tests": [],
  "routes": [],
  "docs": [],
  "last_synced": "2025-07-01",
  "responsible": "José + IA STRATO",
  "coverage": 0,
  "status": "active",
  "criticality": "low"
}
-->

# DEV







## 📁 ARCHIVOS CLAVE

### **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/upwork-transcriber/executor.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/mturk-labeler/executor.ts` - Archivo fuente
- `scripts/agents/n8n-microservice/executor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/freelancer-leadgen/executor.ts` - Archivo fuente
- `scripts/agents/fiverr-writer/executor.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/upwork-transcriber/executor.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/mturk-labeler/executor.test.ts` - Archivo de test
- `scripts/agents/n8n-microservice/executor.test.ts` - Archivo de test
- `scripts/agents/freelancer-leadgen/executor.test.ts` - Archivo de test
- `scripts/agents/fiverr-writer/executor.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/data/README.md` - Archivo de documentación
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/find-bad-routes.js` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/orchestrator.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/support/analyze.ts` - Archivo fuente
- `scripts/agents/security/security-check.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/autotest.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/perf/benchmark.ts` - Archivo fuente
- `scripts/agents/merge-strategist/plan-merge.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/licenses/validate-licenses.ts` - Archivo fuente
- `scripts/agents/i18n/detect.ts` - Archivo fuente
- `scripts/agents/env/validate-env.ts` - Archivo fuente
- `scripts/agents/docs/docgen.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/data/backup.ts` - Archivo fuente

### **Test Files**
- `scripts/orchestrator.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/support/__tests__/analyze.test.ts` - Archivo de test
- `scripts/agents/security/__tests__/security-check.test.ts` - Archivo de test
- `scripts/agents/refactor/__tests__/autofix.test.ts` - Archivo de test
- `scripts/agents/qa/__tests__/autotest.test.ts` - Archivo de test
- `scripts/agents/perf/__tests__/benchmark.test.ts` - Archivo de test
- `scripts/agents/merge-strategist/__tests__/plan-merge.test.ts` - Archivo de test
- `scripts/agents/licenses/__tests__/validate-licenses.test.ts` - Archivo de test
- `scripts/agents/i18n/__tests__/detect.test.ts` - Archivo de test
- `scripts/agents/env/__tests__/validate-env.test.ts` - Archivo de test
- `scripts/agents/docs/__tests__/docgen.test.ts` - Archivo de test
- `scripts/agents/data/__tests__/backup.test.ts` - Archivo de test
- `scripts/agents/context-watchdog/__tests__/context-watchdog.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/data/index.test.ts` - Archivo de test
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/data/README.md` - Archivo de documentación
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/data/index.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test
- `scripts/agents/data/index.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación
- `scripts/agents/data/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/qa/run-qa.ts` - Archivo fuente
- `scripts/agents/qa/index.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**
- `scripts/agents/qa/run-qa.test.ts` - Archivo de test
- `scripts/agents/qa/index.test.ts` - Archivo de test
- `scripts/agents/qa/audit.test.ts` - Archivo de test

### **Config Files**


### **Doc Files**
- `scripts/agents/qa/README.md` - Archivo de documentación## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/qa/audit.ts` - Archivo fuente
- `scripts/agents/merge-strategist/conflict-resolver.ts` - Archivo fuente
- `scripts/agents/data/processor.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/defense-reminder.ts` - Archivo fuente
- `scripts/defense-pack-implementation.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/auto-documentation.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## **Source Files**
- `scripts/visibility-report.ts` - Archivo fuente
- `scripts/validate-traceability.ts` - Archivo fuente
- `scripts/validate-test-templates.ts` - Archivo fuente
- `scripts/validate-repo.ts` - Archivo fuente
- `scripts/validate-module.ts` - Archivo fuente
- `scripts/validate-env.ts` - Archivo fuente
- `scripts/taskmaster.prd.ts` - Archivo fuente
- `scripts/sync-modules-index.ts` - Archivo fuente
- `scripts/report-strato-defense.ts` - Archivo fuente
- `scripts/report-status.ts` - Archivo fuente
- `scripts/pre-commit-validation.ts` - Archivo fuente
- `scripts/night-watchman.ts` - Archivo fuente
- `scripts/init-project.ts` - Archivo fuente
- `scripts/generate-code.ts` - Archivo fuente
- `scripts/db-seed.ts` - Archivo fuente
- `scripts/create-module.ts` - Archivo fuente
- `scripts/check-react-imports.ts` - Archivo fuente
- `scripts/check-env.ts` - Archivo fuente
- `scripts/check-blindaje-real.ts` - Archivo fuente
- `scripts/audit-visibility.ts` - Archivo fuente
- `scripts/audit-module.ts` - Archivo fuente
- `scripts/audit-checklist.ts` - Archivo fuente
- `scripts/angel-vigilante.ts` - Archivo fuente
- `scripts/audit/audit-system.ts` - Archivo fuente
- `scripts/agents/strato.logic.ts` - Archivo fuente
- `scripts/agents/context-watchdog.watch.ts` - Archivo fuente
- `scripts/agents/context-watchdog.ts` - Archivo fuente
- `scripts/agents/security/audit.ts` - Archivo fuente
- `scripts/agents/refactor/autofix.ts` - Archivo fuente

### **Test Files**


### **Config Files**


### **Doc Files**
## Source Files
- `scripts/visibility-report.ts`
- `scripts/validate-traceability.ts`
- `scripts/validate-test-templates.ts`
- `scripts/validate-repo.ts`
- `scripts/validate-module.ts`
- `scripts/validate-env.ts`
- `scripts/taskmaster.prd.ts`
- `scripts/sync-modules-index.ts`
- `scripts/report-strato-defense.ts`
- `scripts/report-status.ts`
- `scripts/pre-commit-validation.ts`
- `scripts/night-watchman.ts`
- `scripts/init-project.ts`
- `scripts/generate-code.ts`
- `scripts/db-seed.ts`
- `scripts/create-module.ts`
- `scripts/check-react-imports.ts`
- `scripts/check-env.ts`
- `scripts/check-blindaje-real.ts`
- `scripts/audit-visibility.ts`
- `scripts/audit-module.ts`
- `scripts/audit-checklist.ts`
- `scripts/angel-vigilante.ts`
- `scripts/audit/audit-system.ts`
- `scripts/agents/strato.logic.ts`
- `scripts/agents/context-watchdog.watch.ts`
- `scripts/agents/context-watchdog.ts`
- `scripts/agents/security/audit.ts`
- `scripts/agents/refactor/autofix.ts`

### Test Files


### Config Files


### Doc Files


### Scripts

### Source Files
- `scripts/visibility-report.ts`
- `scripts/validate-traceability.ts`
- `scripts/validate-test-templates.ts`
- `scripts/validate-repo.ts`
- `scripts/validate-module.ts`
- `scripts/validate-env.ts`
- `scripts/taskmaster.prd.ts`
- `scripts/sync-modules-index.ts`
- `scripts/report-strato-defense.ts`
- `scripts/report-status.ts`
- `scripts/pre-commit-validation.ts`
- `scripts/night-watchman.ts`
- `scripts/init-project.ts`
- `scripts/generate-code.ts`
- `scripts/db-seed.ts`
- `scripts/create-module.ts`
- `scripts/check-react-imports.ts`
- `scripts/check-env.ts`
- `scripts/check-blindaje-real.ts`
- `scripts/audit-visibility.ts`
- `scripts/audit-module.ts`
- `scripts/audit-checklist.ts`
- `scripts/angel-vigilante.ts`
- `scripts/audit/audit-system.ts`
- `scripts/agents/strato.logic.ts`
- `scripts/agents/context-watchdog.watch.ts`
- `scripts/agents/context-watchdog.ts`
- `scripts/agents/security/audit.ts`
- `scripts/agents/refactor/autofix.ts`

### Test Files


### Config Files


### Doc Files


### Scripts

### Source Files
- `scripts/visibility-report.ts`
- `scripts/validate-traceability.ts`
- `scripts/validate-test-templates.ts`
- `scripts/validate-repo.ts`
- `scripts/validate-module.ts`
- `scripts/validate-env.ts`
- `scripts/taskmaster.prd.ts`
- `scripts/sync-modules-index.ts`
- `scripts/report-strato-defense.ts`
- `scripts/report-status.ts`
- `scripts/pre-commit-validation.ts`
- `scripts/night-watchman.ts`
- `scripts/init-project.ts`
- `scripts/generate-code.ts`
- `scripts/db-seed.ts`
- `scripts/create-module.ts`
- `scripts/check-react-imports.ts`
- `scripts/check-env.ts`
- `scripts/check-blindaje-real.ts`
- `scripts/audit-visibility.ts`
- `scripts/audit-module.ts`
- `scripts/audit-checklist.ts`
- `scripts/angel-vigilante.ts`
- `scripts/agents/strato.logic.ts`
- `scripts/agents/context-watchdog.watch.ts`
- `scripts/agents/context-watchdog.ts`
- `scripts/audit/audit-system.ts`
- `scripts/agents/security/audit.ts`
- `scripts/agents/refactor/autofix.ts`

### Test Files


### Config Files


### Doc Files


### Scripts

### Source Files
- `scripts/visibility-report.ts`
- `scripts/validate-traceability.ts`
- `scripts/validate-test-templates.ts`
- `scripts/validate-repo.ts`
- `scripts/validate-module.ts`
- `scripts/validate-env.ts`
- `scripts/taskmaster.prd.ts`
- `scripts/sync-modules-index.ts`
- `scripts/report-strato-defense.ts`
- `scripts/report-status.ts`
- `scripts/pre-commit-validation.ts`
- `scripts/night-watchman.ts`
- `scripts/init-project.ts`
- `scripts/generate-code.ts`
- `scripts/db-seed.ts`
- `scripts/create-module.ts`
- `scripts/check-react-imports.ts`
- `scripts/check-env.ts`
- `scripts/check-blindaje-real.ts`
- `scripts/audit-visibility.ts`
- `scripts/audit-module.ts`
- `scripts/audit-checklist.ts`
- `scripts/angel-vigilante.ts`
- `scripts/audit/audit-system.ts`
- `scripts/agents/strato.logic.ts`
- `scripts/agents/context-watchdog.watch.ts`
- `scripts/agents/context-watchdog.ts`
- `scripts/agents/refactor/autofix.ts`
- `scripts/agents/security/audit.ts`

### Test Files


### Config Files


### Doc Files


### Scripts

### Source Files
- `scripts/validate-traceability.ts`
- `scripts/validate-test-templates.ts`
- `scripts/validate-repo.ts`
- `scripts/validate-module.ts`
- `scripts/validate-env.ts`
- `scripts/taskmaster.prd.ts`
- `scripts/sync-modules-index.ts`
- `scripts/report-strato-defense.ts`
- `scripts/report-status.ts`
- `scripts/pre-commit-validation.ts`
- `scripts/night-watchman.ts`
- `scripts/init-project.ts`
- `scripts/generate-code.ts`
- `scripts/db-seed.ts`
- `scripts/create-module.ts`
- `scripts/check-react-imports.ts`
- `scripts/check-env.ts`
- `scripts/check-blindaje-real.ts`
- `scripts/audit-visibility.ts`
- `scripts/audit-module.ts`
- `scripts/audit-checklist.ts`
- `scripts/angel-vigilante.ts`
- `scripts/agents/strato.logic.ts`
- `scripts/agents/context-watchdog.watch.ts`
- `scripts/agents/context-watchdog.ts`
- `scripts/audit/audit-system.ts`
- `scripts/agents/security/audit.ts`
- `scripts/agents/refactor/autofix.ts`

### Test Files


### Config Files


### Doc Files


### Scripts

### Source Files
- `scripts/validate-traceability.ts`
- `scripts/validate-test-templates.ts`
- `scripts/validate-repo.ts`
- `scripts/validate-module.ts`
- `scripts/validate-env.ts`
- `scripts/taskmaster.prd.ts`
- `scripts/sync-modules-index.ts`
- `scripts/report-strato-defense.ts`
- `scripts/report-status.ts`
- `scripts/pre-commit-validation.ts`
- `scripts/night-watchman.ts`
- `scripts/init-project.ts`
- `scripts/generate-code.ts`
- `scripts/db-seed.ts`
- `scripts/create-module.ts`
- `scripts/check-react-imports.ts`
- `scripts/check-env.ts`
- `scripts/check-blindaje-real.ts`
- `scripts/audit-visibility.ts`
- `scripts/audit-module.ts`
- `scripts/audit-checklist.ts`
- `scripts/angel-vigilante.ts`
- `scripts/audit/audit-system.ts`
- `scripts/agents/strato.logic.ts`
- `scripts/agents/context-watchdog.watch.ts`
- `scripts/agents/context-watchdog.ts`
- `scripts/agents/refactor/autofix.ts`
- `scripts/agents/security/audit.ts`

### Test Files


### Config Files


### Doc Files


### Scripts

### Source Files

### Test Files

### Config Files

### Doc Files

### Scripts

---

## Descripción

Utilidades, scripts y helpers para el desarrollo y mantenimiento del monorepo STRATO.

## Checklist de trazabilidad
- [ ] Header JSON válido
- [ ] Archivos clave listados
- [ ] Tests asociados
- [ ] Rutas documentadas
- [ ] Sin archivos huérfanos 